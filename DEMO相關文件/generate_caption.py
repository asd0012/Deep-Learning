...
def generate_caption_visualization(encoder, decoder, img_path, word_dict, beam_size=3, smooth=True):
    ...
    for idx in range(1, num_words):
        if idx == 1:
            target += sentence_tokens[idx][0].upper()+sentence_tokens[idx][1:]
        elif idx == num_words-1:
            target += '.'
        else:
            target += ' '+sentence_tokens[idx]
    plt.imshow(img)
    plt.axis('off')
    plt.text(0, 1, target, backgroundcolor='white', fontsize=13)
    plt.text(0, 1, target, color='black', fontsize=13)
    
    plt.savefig(img_path.split('.')[0]+'_result.jpg', bbox_inches='tight')
    plt.show()
    ...
    plt.savefig(img_path.split('.')[0]+'_detail.jpg')
    plt.show()


if __name__ == "__main__":
    img_path_default = 'target/test.jpg'
    model_path_default = 'model/model_10.pth'
    ...
    parser.add_argument('--img-path', type=str, default=img_path_default,
                        help='path to image')
    parser.add_argument('--model', type=str, default=model_path_default,
                        help='path to model paramters')
    ...
    decoder.load_state_dict(torch.load(args.model, map_location='cpu'))
